// // import 'dart:math';
// //
// // import 'package:faker/faker.dart';
// // import 'package:quick_social/models/models.dart';
// //
// // class Post {
// //   final User owner;
// //   final String postImage;
// //   final String location;
// //   final String caption;
// //   final List<Comment> comments;
// //   final int likeCount;
// //   final int saveCount;
// //   final bool isLiked;
// //   final bool isSaved;
// //
// //   const Post({
// //     required this.owner,
// //     required this.postImage,
// //     required this.location,
// //     required this.caption,
// //     required this.comments,
// //     required this.likeCount,
// //     required this.saveCount,
// //     this.isLiked = false,
// //     this.isSaved = false,
// //   });
// //
// //   static final List<Post> dummyPosts = List.generate(
// //     30,
// //     (index) {
// //       final Faker faker = Faker();
// //       return Post(
// //         owner: User.dummyUsers[
// //             index > 15 ? 0 : Random().nextInt(User.dummyUsers.length - 1)],
// //         postImage: faker.image.loremPicsum(
// //           random: Random().nextInt(30),
// //           height: 640,
// //           width: 640,
// //         ),
// //         location: '${faker.address.city()}, ${faker.address.country()}',
// //         caption: faker.lorem.sentence(),
// //         comments: Comment.generateDummyComments(),
// //         likeCount: Random().nextInt(1000),
// //         saveCount: Random().nextInt(1000),
// //       );
// //     },
// //   );
// // }
// mốt đổi lại test duùm
// import 'package:quick_social/models/user.dart';
//
// class Post {
//   final String caption;
//   final String media;
//   final User user;
//
//   Post({required this.caption, required this.media, required this.user});
//
//   factory Post.fromJson(Map<String, dynamic> json) {
//     return Post(
//       caption: json['caption'],
//       media: json['media'],
//       user: User.fromJson(json['user']),
//     );
//   }
// }
//
//


import 'package:quick_social/models/userinfo.dart';

class Post {
  final String id;
  final String caption;
  final UserInfo user;
  final int likeCount;
  final int saveCount;
  final int shareCount;
  final String media;
  final List<String> comments; // Comments có thể là List hoặc null

  Post({
    required this.id,
    required this.caption,
    required this.user,
    required this.likeCount,
    required this.saveCount,
    required this.shareCount,
    required this.media,
    required this.comments,
  });

  factory Post.fromJson(Map<String, dynamic> json) {
    // Kiểm tra và thay thế null bằng một danh sách rỗng
    var comments = json['comments'];
    return Post(
      id: json['id'],
      caption: json['caption'],
      user: UserInfo.fromJson(json),
      likeCount: json['likeCount'],
      saveCount: json['saveCount'],
      shareCount: json['shareCount'],
      media: json['media'],
      comments: comments != null ? List<String>.from(comments) : [], // Xử lý null cho comments
    );
  }
}

